...
contract OmniCoinConfig is Ownable, ReentrancyGuard {
    ...
    // Events
    event BridgeConfigAdded(uint256 indexed chainId, address token, uint256 minAmount, uint256 maxAmount, uint256 fee);
    event BridgeConfigRemoved(uint256 indexed chainId);
    event StakingTierUpdated(uint256 indexed tierId, uint256 minAmount, uint256 maxAmount, uint256 rewardRate, uint256 lockPeriod, uint256 penaltyRate);
    ...
    function isBridgeSupported(uint256 _chainId) external view returns (bool) {
        for (uint256 i = 0; i < bridgeConfigs.length; i++) {
            if (bridgeConfigs[i].chainId == _chainId && bridgeConfigs[i].isActive) {
                return true;
            }
        }
        return false;
    ...
    function getBridgeConfig(uint256 _chainId) external view returns (BridgeConfig memory) {
        for (uint256 i = 0; i < bridgeConfigs.length; i++) {
            if (bridgeConfigs[i].chainId == _chainId) {
    ...
    function updateStakingTier(
        ...
        require(_tierId < stakingTiers.length, "OmniCoinConfig: invalid tier ID");
        ...
        emit StakingTierUpdated(_tierId, _minAmount, _maxAmount, _rewardRate, _lockPeriod, _penaltyRate);
    }
    
    function getStakingTier(uint256 _amount) external view returns (StakingTier memory) {
