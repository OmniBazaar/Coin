 ...
 * @notice Privacy-preserving WBTC wrapper using COTI V2 MPC
 * @dev Wraps real WBTC tokens into MPC-encrypted private balances
 *      (pWBTC) using COTI V2 garbled circuits. WBTC uses 8 decimals;
 *      amounts are scaled down by 1e2 to fit MPC 6-decimal precision.
 *      Scaling factor = 100.
 *
 * Token custody model:
 * - bridgeMint: Receives real WBTC via safeTransferFrom and credits
 *   the recipient's publicBalances mapping.
 * - bridgeBurn: Debits the user's publicBalances and transfers real
 *   WBTC back via safeTransfer.
 * - convertToPrivate: Debits publicBalances (full 8-dec amount),
 *   scales to 6-dec, creates encrypted MPC balance. Dust (remainder
 *   after scaling) is tracked in dustBalances for user refund.
 * - convertToPublic: Decrypts MPC balance, scales back to 8-dec,
 *   and credits publicBalances.
 *
 * Scaling precision:
 * - Maximum rounding dust per conversion: 99 satoshi (~$0.09 at
 *   $90,000/BTC).
 * - Minimum convertible amount: 100 satoshi (0.000001 BTC).
 * - Dust is tracked per user and refundable via claimDust().
 *
 * Max private balance: type(uint64).max * 100 / 1e8 = ~18,446 BTC
 * (sufficient for all practical DEX trades).
 *
 * Features:
 * - Real WBTC custody via SafeERC20 transfers
 * - Per-user public balance tracking
 * - Dust tracking and refund mechanism
 * - Public to private conversion (scales 8 -> 6 decimals)
 * - Private to public conversion (scales 6 -> 8 decimals)
    ...
    UUPSUpgradeable
{
    ...
    /// @notice Scaling factor: 8 decimals to 6 decimals for MPC
    /// @dev WBTC amounts divided by 100 before MPC encryption.
    ///      Maximum rounding dust per conversion: 99 satoshi
    ///      (~$0.09 at $90,000/BTC). Minimum convertible amount:
    ///      100 satoshi (0.000001 BTC).
    uint256 public constant SCALING_FACTOR = 1e2;

    /// @notice Token name for wallet/explorer display
    string public constant TOKEN_NAME = "Private WBTC";

    /// @notice Token symbol for wallet/explorer display
    string public constant TOKEN_SYMBOL = "pWBTC";
    ...
    /// @notice The underlying WBTC token contract held in custody
    ...
    /// @notice Total public supply (bridged in, 8-decimal units)
    uint256 public totalPublicSupply;

    /// @notice Per-user public balance tracking for bridge deposits
    /// @dev Credited by bridgeMint (8-dec), debited by
    ...
    /// @param amount Amount in WBTC units (8 decimals)
    ...
    /// @param user User address
    /// @param amount Dust amount in WBTC units (8 decimals)
    ...
    /**
     * @notice Initialize PrivateWBTC with admin and underlying token
     ...
     * @param _underlyingToken Address of the WBTC token contract
     */
    function initialize(
        address admin,
    ...
    /**
     * @notice Mint tokens from bridge deposit with real WBTC custody
     * @dev Transfers WBTC from msg.sender (bridge) into this contract
     *      and credits the recipient's publicBalances. Requires prior
     *      WBTC approval from msg.sender to this contract.
     * @param to Recipient address
     * @param amount Amount in WBTC units (8 decimals)
     */
    function bridgeMint(
    ...
    /**
     * @notice Burn tokens for bridge withdrawal with real WBTC release
     * @dev Debits the user's publicBalances and transfers real WBTC
    ...
    /**
     * @notice Convert public WBTC to private pWBTC
     * @dev Scales 8 decimals down to 6 decimals for MPC storage.
     ...
     * @param amount Amount of WBTC in satoshis (8 decimals)
     */
    function convertToPrivate(
        ...
        // Scale down: 8 decimals -> 6 decimals
    ...
    /**
     * @notice Convert private pWBTC back to public WBTC
     * @dev Decrypts MPC amount and scales back to 8 decimals.
    ...
    /**
     * @notice Transfer private pWBTC to another address
     ...
     *      scales it back to 8 decimals, and credits publicBalances.
     *
    ...
    function emergencyRecoverPrivateBalance(
        ...
        // Scale back to 8-decimal and credit public balance
     ...
     *      tracking that could erode privacy guarantees. BTC's high
     *      visibility makes correlation attacks more practical.
     ...
     * @return Number of decimals (8, matching WBTC)
     */
