 ...
 *      - averageRating: scaled ×100 (e.g., 450 = 4.50 stars)
...
contract ReputationCredential is ERC721 {
    ...
    // ── Events ─────────────────────────────────────────────────────────
    /// @dev ERC-5192: Emitted when a token's locked status changes.
    ...
    struct ReputationData {
        ...
        uint16 averageRating;      // ×100 scale (0-500)
    ...
    // ── State ──────────────────────────────────────────────────────────
    /// @dev Address authorized to mint and update reputation data.
    address public immutable AUTHORIZED_UPDATER;

    /// @dev Auto-incrementing token ID counter.
    uint256 private _nextTokenId;

    /// @dev tokenId → ReputationData.
    mapping(uint256 => ReputationData) private _reputation;

    /// @dev userAddress → tokenId (0 means no token).
    mapping(address => uint256) private _userToken;

    // ── Constructor ────────────────────────────────────────────────────
    /**
     * @notice Deploy the ReputationCredential contract.
     * @param authorizedUpdater Address allowed to mint and update (validator deployer key).
     */
    constructor(
        address authorizedUpdater
    ) ERC721("OmniBazaar Reputation", "OMNI-REP") {
        AUTHORIZED_UPDATER = authorizedUpdater;
    ...
    modifier onlyUpdater() {
        if (msg.sender != AUTHORIZED_UPDATER) revert NotAuthorized();
        _;
    }
    ...
    function mint(
        ...
        emit Locked(tokenId);
        emit ReputationUpdated(tokenId, data.participationScore);
    }
    ...
    /**
     * @dev Build a single ERC-721 metadata attribute JSON fragment.
