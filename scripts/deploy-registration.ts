/**
 * @file deploy-registration.ts
 * @description Deployment script for OmniRegistration contract
 *
 * Usage:
 *   npx hardhat run scripts/deploy-registration.ts --network fuji
 *   npx hardhat run scripts/deploy-registration.ts --network localhost
 *
 ...
 *
 * Post-Deployment:
 *   - Grant VALIDATOR_ROLE to gateway validators
 *   - Grant KYC_ATTESTOR_ROLE to trusted KYC validators
 *   - Set OmniRewardManager to call markWelcomeBonusClaimed
 */
...
function saveDeploymentConfig(network: string, result: DeploymentResult): void {
    ...
    config.OmniRegistration = result.proxyAddress;
    config.OmniRegistrationImpl = result.implementationAddress;
...
async function main(): Promise<void> {
    console.log('========================================');
    console.log('OmniRegistration Deployment');
    console.log('========================================\n');
    ...
    // Deploy OmniRegistration as UUPS proxy
    console.log('\n--- Deploying OmniRegistration ---');
    ...
    // Print next steps
    console.log('\n--- Next Steps ---');
    console.log('1. Grant VALIDATOR_ROLE to gateway validators:');
    console.log(`   const VALIDATOR_ROLE = await contract.VALIDATOR_ROLE();`);
    console.log(`   await contract.grantRole(VALIDATOR_ROLE, validatorAddress);`);
    console.log('\n2. Grant KYC_ATTESTOR_ROLE to trusted validators:');
    console.log(`   const KYC_ATTESTOR_ROLE = await contract.KYC_ATTESTOR_ROLE();`);
    console.log(`   await contract.grantRole(KYC_ATTESTOR_ROLE, attestorAddress);`);
    console.log('\n3. Set OmniRewardManager to call markWelcomeBonusClaimed');
    console.log('========================================\n');
}
